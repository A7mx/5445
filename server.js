...
io.on("connection", (socket) => {
  ...
  socket.on("chat", async ({ toId, message }) => {
    ...
    io.to(toId).emit("chat", { from: fromId, message, timestamp: Date.now() });
    io.to(fromId).emit("chat", { from: fromId, message, timestamp: Date.now() });
  });
});
    ...
    if (userDoc.exists()) {
      ...
      for (const friendId of friends) {
        ...
        chatDocs.forEach((doc) => {
          ...
          if (message.userIds.includes(friendId)) io.to(userId).emit("chat", { from: message.from, message: message.message, timestamp: message.timestamp?.seconds * 1000 });
        });
      }
    }
  } catch (error) {}
